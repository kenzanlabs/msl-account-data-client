...
public class ArtistsByUserQueryTest {
  ...
  public void init() {
    ...
    when(queryAccessor.artistsByUser(tc.USER_ID, tc.TIMESTAMP, tc.ARTIST_ID)).thenThrow(
        new RuntimeException("TEST_EXPECTED_EXCEPTION"));
  }

  @Test(expected = RuntimeException.class)
  public void testGetArtistsByUser() {
    ArtistsByUserQuery.getUserArtist(queryAccessor, mappingManager, tc.USER_ID,
        Long.toString(LONG_TIMESTAMP), tc.ARTIST_ID);
  }

  @Test
  public void testGetArtistListByUser() {
    ArtistsByUserQuery.getUserArtistList(queryAccessor, tc.USER_ID, Optional.absent(),
        Optional.absent());
  ...
  public void testGetArtistListByUserWithLimitAndTimestamp() {
    ArtistsByUserQuery.getUserArtistList(queryAccessor, tc.USER_ID,
  ...
  public void testAddArtistByUser() {
    ...
    verify(queryAccessor, atLeastOnce()).addLibraryArtist(eq(tc.ARTIST_BY_USER_DTO.getUserId()),
        eq(tc.ARTIST_BY_USER_DTO.getContentType()), any(Date.class),
        eq(tc.ARTIST_BY_USER_DTO.getArtistId()), eq(tc.ARTIST_BY_USER_DTO.getArtistMbid()),
        eq(tc.ARTIST_BY_USER_DTO.getArtistName()), eq(tc.ARTIST_BY_USER_DTO.getImageLink()));
  }
  ...
  public void testRemoveArtistByUser() {
    ...
    verify(queryAccessor, atLeastOnce())
        .deleteLibraryArtist(tc.ARTIST_ID, tc.TIMESTAMP, tc.USER_ID);
  }
