...
public class ArtistsByUserQuery {
  ...
  public static Optional<ArtistsByUserDto> getUserArtist(final QueryAccessor queryAccessor,
      final MappingManager manager, final UUID userId, final String timestamp, final UUID artistUuid) {
    Result<ArtistsByUserDto> results =
        manager.mapper(ArtistsByUserDto.class).map(
            queryAccessor.artistsByUser(userId, new Date(Long.parseLong(timestamp)), artistUuid));
    if (results != null) {
  ...
  public static void remove(final QueryAccessor queryAccessor, final UUID userId,
      final Date timestamp, final UUID artistUuid) {
